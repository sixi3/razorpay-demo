/**
 * Snapshot of the drawer PDP format (DetailView).
 * Reference copy — not wired into the app. See pdp-format.css for styles.
 */

import { formatPrice, discountPct } from '../src/data/items';

interface PdpProductLike {
  brand: string;
  name: string;
  rating: number;
  price: number;
...
export function PdpMainPieceExample({
  product,
  size,
  onSizeChange,
}: {
  product: PdpProductLike & { imageUrl: string; sizes: string[] };
  size: string;
...
export function PdpSimilarProductExample({ product }: { product: PdpProductLike & { imageUrl: string } }) {
  return (
    <>
      <div className="pdp">
        <div className="pdp__media">
